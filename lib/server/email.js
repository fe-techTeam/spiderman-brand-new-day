...
// Images must resolve for RECIPIENTS, so they always come off the production
// domain (layout.jsx SITE_URL) — NEXT_PUBLIC_APP_URL is localhost in dev and
// would show broken images in test sends. Links still use the app URL.
const ASSET_BASE = "https://spidermania.in";
...
export function renderBrandEmail({ title, preheader = "", eyebrow = "Brand New Day", bodyHtml, ctaLabel, ctaUrl, footnoteHtml = "" }) {
  const site = process.env.NEXT_PUBLIC_APP_URL || ASSET_BASE;
  const p = (inner, extra = "") =>
    `<p style="margin:0 0 18px; font-family:${T.bodyFont}; font-size:15px; line-height:1.65; color:${T.text}; ${extra}">${inner}</p>`;
          ...
          <tr>
            <td align="center" style="padding:0 0 24px;">
              <a href="${site}" style="text-decoration:none;">
                <img src="${ASSET_BASE}/assets/nav-logo.png" width="170" alt="Spider-Man: Brand New Day" class="em-logo" style="display:block; width:170px; max-width:60%; height:auto; border:0;" />
              </a>
            </td>
          </tr>
...
export async function sendPasswordResetEmail({ to, resetUrl, admin = false }) {
  ...
  const html = renderBrandEmail({
    ...
    bodyHtml:
      emailParagraph(`Someone (hopefully you) asked to reset the ${who} password for this email address.`) +
  ...
  const text = `Someone (hopefully you) asked to reset the ${who} password for this email address.
