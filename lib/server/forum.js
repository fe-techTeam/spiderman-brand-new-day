...
export function postDTO(row, media = []) {
  return {
    ...
    // avatarPic: identity-wide profile picture from the avatars master
    author: { username: row.username, avatarPic: row.author_avatar_pic || null },
...
/** Re-sync a post's denormalized comment_count to its live active-comment count.
 *  Create/author-delete keep the counter with cheap +1/-N, but moderation
 *  (hide / unhide / report takedown) changes a comment's status without touching
 *  the counter — so the forum card badge would read "2 Comments" while the thread
 *  renders only 1. Recomputing from the source of truth after any status change
 *  keeps the card badge and the thread's live total in lock-step. Runs on the
 *  caller's transaction connection so it stays atomic with the status update. */
export async function recountPostComments(conn, postId) {
  await conn.execute(
    `UPDATE posts SET comment_count =
       (SELECT COUNT(*) FROM comments WHERE post_id = ? AND status = 'active')
     WHERE id = ?`,
    [postId, postId]
  );
}
