...
/** Structural sanity check for videos — magic bytes alone pass truncated
    files. For MP4/MOV, walk the top-level box chain: every box's declared
    size must land exactly on the next box (and finally on EOF), and a
    playable file must carry a `moov` plus media data (`mdat`, or `moof`
    for fragmented files). A cut-off upload breaks the chain or loses those
    boxes. WebM can't be checked this cheaply (EBML elements may declare
    unknown size for streaming), so it passes on its sniffed header alone. */
export function validateVideo(buffer, sig) {
  if (sig.mime !== "video/mp4") return true;
  let off = 0;
  const types = new Set();
  while (off < buffer.length) {
    if (off + 8 > buffer.length) return false;
    let size = buffer.readUInt32BE(off);
    const type = buffer.toString("latin1", off + 4, off + 8);
    if (!/^[\x20-\x7e]{4}$/.test(type)) return false;
    types.add(type);
    if (size === 0) {
      off = buffer.length; // final box extends to EOF — valid per spec
      break;
    }
    if (size === 1) {
      if (off + 16 > buffer.length) return false;
      const large = buffer.readBigUInt64BE(off + 8);
      if (large < 16n || large > BigInt(buffer.length)) return false;
      size = Number(large);
    } else if (size < 8) {
      return false;
    }
  ...
  if (off !== buffer.length) return false;
  return types.has("moov") && (types.has("mdat") || types.has("moof"));
}
...
export async function saveUpload(file, subdir, { allowVideo = false } = {}) {
  ...
  if (sig.kind === "video" && !validateVideo(buffer, sig)) {
    throw new Error("Video file looks corrupt or incomplete");
  }
