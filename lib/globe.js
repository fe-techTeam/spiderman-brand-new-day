...
export function initGlobe(cv, { getTwinActive } = {}) {
  ...
  // Red map hubs/pings: a deliberately sparse set (~30% of the city list) with
  // none in Europe or Africa, so the twin globe reads calmer and stays on the
  // Americas, Asia, and Oceania. (The Living Web ticker still uses the full
  // CITIES list — this only trims the red dots drawn on the map.)
  const HUB_CITIES = ["New York, USA", "São Paulo, Brazil", "Mumbai, India", "Tokyo, Japan", "Sydney, Australia"];
  const cityCoords = HUB_CITIES.map((name) => CITY_LL[name] || [0, 0]);
  ...
  const build = () => {
    ...
    // Secondary red blooms — also cut ~70% and stripped of every Europe/Africa
    // point, matching the sparser hub set above.
    const extraLL = [
      [-118, 34], [-87.6, 41.8], [103.8, 1.35], [121.5, 31.2], [126.9, 37.6], [-58.4, -34.6], [-70.7, -33.5],
    ];
    globeExtra = extraLL.map(([lon, lat]) => proj(lon, lat, W, H));
    twinHomePx = proj(72.8, 19.1, W, H);
    twinDestPx = [[-46.6, -23.5], [139.7, 35.7], [151.2, -33.9], [-74, 40.7]].map(([lo, la]) => proj(lo, la, W, H));
    globeHubs = hubsPx;
  };
  build();
  ...
  // only draw while the canvas is on screen — offscreen it was burning a full
  // rAF of gradients + shadowBlur against every other animation on the page
  let vizObs = null;
