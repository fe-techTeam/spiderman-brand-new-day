...
| `username`          | VARCHAR(30) NOT NULL                                | **UNIQUE**. Stored _without_ the `u/` prefix (UI renders `u/{username}`). Charset-restricted `[a-z0-9_]{3,30}`, lowercased — this keeps the frontend's `@mention` regex (`@[A-Za-z0-9_]+`) working. |
...
**This table _is_ the avatar-assignment mapping** the admin panel edits. Changing a row changes the
...
audit trail of _why_ a user got their avatar.
...
in the mocks are _computed_ badges, not stored flairs — see §7.4.)
...
Indexes: `(status, id DESC)` — the review queue _and_ the public gallery are both this index.
...
A portal JWT can never be replayed against admin APIs (different secret _and_ different cookie
name _and_ different verification path).
...
rows: the extra row's existence _is_ `hasMore` — no `COUNT(*)` ever runs on hot paths.
...
### Phase 0 — Foundations _(built alongside this document)_
...
### Phase 7 — Admin panel _(started alongside this document)_
